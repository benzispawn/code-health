...
export function readLcovCoverage(cwd: string, lcovPath = 'coverage/lcov.info'): Map<string, CoverageEntry> {
  ...
  for (const record of records) {
    ...
    coverage.set(sourceFile, {
      lineCoverage: foundLines === undefined || foundLines === 0 || hitLines === undefined
        ? undefined
        : Math.round((hitLines / foundLines) * 100),
      branchCoverage: foundBranches === undefined || foundBranches === 0 || hitBranches === undefined
        ? undefined
