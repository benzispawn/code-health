...
export function calculateHealthSummary(
  ...
  const testabilityScore = average(files.map((file) => (file.metrics.lineCoverage === undefined ? 50 : file.metrics.lineCoverage)));
  ...
  return {
    ...
    duplicationPercent: Math.round(average(files.map((file) => file.metrics.duplicationPercent))),
    maxDependencyDepth: Math.max(0, ...files.map((file) => file.metrics.dependencyDepth)),
    averageLineCoverage: lineCoverageValues.length === 0 ? undefined : Math.round(average(lineCoverageValues)),
    averageBranchCoverage: branchCoverageValues.length === 0 ? undefined : Math.round(average(branchCoverageValues)),
    apiSurfaceSize: files.reduce(
      (total, file) => total + file.metrics.publicExportCount + file.metrics.controllerCount + file.metrics.endpointCount,
      0,
    ),
    publicExportCount: files.reduce((total, file) => total + file.metrics.publicExportCount, 0),
    controllerCount: files.reduce((total, file) => total + file.metrics.controllerCount, 0),
    endpointCount: files.reduce((total, file) => total + file.metrics.endpointCount, 0),
...
function calculateFileScore(file: FileAnalysis, config: CodeHealthConfig): number {
  ...
  score -= overThresholdPenalty(file.metrics.duplicationPercent, config.thresholds.duplicationPercent, 1.5);
