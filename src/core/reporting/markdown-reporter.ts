...
export function formatMarkdownReport(report: ProjectHealthReport): string {
  const lines: string[] = [
    ...
    ...(report.summary.averageLineCoverage === undefined ? [] : [`- Line Coverage: ${report.summary.averageLineCoverage}%`]),
    ...(report.summary.averageBranchCoverage === undefined ? [] : [`- Branch Coverage: ${report.summary.averageBranchCoverage}%`]),
    '',
    '## Critical Findings',
    '',
  ];
  ...
  lines.push('', '## Duplication', '', `Project Duplication: ${report.duplication.percent}%`, '');
    ...
    lines.push(
      ...report.duplication.groups.slice(0, 20).map((group) => {
        const occurrences = group.occurrences
          .map((occurrence) => `${occurrence.file}:${occurrence.lineStart}-${occurrence.lineEnd}`)
          .join('<br>');
  ...
  if (report.files.length === 0) {
    ...
    lines.push('| File | Score | LOC | Logical LOC | Comments | Duplication | Fan-in | Fan-out | Depth | Exports | Endpoints | Coverage |');
    lines.push('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |');
    lines.push(
      ...report.files
        .slice(0, 30)
        .map((file) => {
          const coverage = file.metrics.lineCoverage === undefined ? '-' : `${file.metrics.lineCoverage}%`;
