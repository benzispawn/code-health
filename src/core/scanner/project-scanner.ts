...
function calculateDomains(
  ...
  const domainNames = [...new Set(files.map((file) => file.domain).filter((domain): domain is string => Boolean(domain)))];
  ...
  return domainNames.sort().map((name) => {
    ...
    return {
      ...
      averageScore: Math.round(domainFiles.reduce((total, file) => total + file.score, 0) / domainFiles.length),
      architectureViolations: violations.filter((violation) => {
        const file = files.find((candidate) => candidate.path === violation.file);
